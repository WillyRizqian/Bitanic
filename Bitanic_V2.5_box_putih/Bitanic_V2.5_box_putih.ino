...
// Description  : Development process of Nutriferads
...
// Panel Declaration
String project = "Nutriferads";
String id = "NF05";
String clientID = "ferads " + id;
...
// Publis Topic
String publisTopic = "ferads";
// Subscribe Topic
String subscribeTopic = "ferads/" + id;
...
#define RELAY2 15
#define BUZZER 27
#define soilMoisture1 35
#define soilMoisture2 32
#define analogPin 34
...
Task dataUpdate(1000, []() {
  ...
  motor1Value = digitalRead(RELAY1) == 1 ? 0 : 1;
  motor2Value = digitalRead(RELAY2) == 1 ? 0 : 1;
  // = = = = = = = = = = = = = DHT11 = = = = = = = = = = = = =
...
Task lcdUpdate(1500, []() {
  ...
  if (millis() - lcdMilis >= 1500)
  {
    ...
    {
      lcdPrint("POMPA 1 : " + motor1Status, "POMPA 2 : " + motor2Status);
      lcdQueue = 3;
    }
    else if (lcdQueue == 3)
    {
...
void Send() {
  ...
  doc["STATUS_MOTOR1"] = digitalRead(RELAY1) == 1 ? 0 : 1;
  doc["STATUS_MOTOR2"] = digitalRead(RELAY2) == 1 ? 0 : 1;
    ...
    if (c == ',') {
      ...
      if (DATA[0] == id) {
        ...
        if (DATA[1] == "1") {
          ...
          lcdPrint("POMPA 2", "Hidup");
          sendMotorStatus(id, motor1Value, motor2Value, publisTopic);
          buzz(500, 3);
        } else {
          digitalWrite(RELAY2, HIGH);
          digitalWrite(led2, LOW);
          lcdPrint("POMPA 2", "Mati");
          sendMotorStatus(id, motor1Value, motor2Value, publisTopic);
          buzz(500, 3);
        }
...
void setup() {
  ...
  lcdPrint("Nutriferads", id);
  buzz(100, 2);
  buzz(500, 1);
  String wifiAP = "Nutriferads " + id;
  lcdPrint(wifiAP.c_str(), "192.168.4.1");
...
void loop() {
  ...
  if (soilmoisture2Value == HIGH) {
    digitalWrite(RELAY2, LOW);  // Menghidupkan relay jika kelembaban terdeteksi
    sendMotorStatus(id, motor1Value, 1, publisTopic);
    Send();
    digitalWrite(led2, HIGH);
    delay (1000);
  } else {
    digitalWrite(RELAY2, HIGH); // Mematikan relay jika kelembaban tidak terdeteksi
